...
@Pipe({
  name: 'psxbCompatability'
})
export class PsxbCompatabilityPipe implements PipeTransform {
    ...
    return videogames.filter(function(videogame) {
      if (videogame.available_on.length > 1) {
        for (var i = 0; i < videogame.available_on.length; i++) {
          if (videogame.available_on[i].platform_name.toLowerCase().includes('xbox')) {
            for (var j = 0; j < videogame.available_on[i].compatible_with.length; j++) {
              if (videogame.available_on[i].compatible_with[j].platform_name.toLowerCase().includes('playstation')) {
